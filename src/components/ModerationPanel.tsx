...
export default function ModerationPanel({ members, roles, onUpdateMember, onRemoveMember }: ModerationPanelProps) {
  ...
  return (
    <div id="moderation-panel" className="bg-zinc-950/80 border border-cyan-500/30 p-6 rounded-2xl shadow-[0_0_20px_rgba(6,182,212,0.15)] relative overflow-hidden">
      ...
      {/* Members table grid */}
      <div className="z-10 border border-cyan-500/10 rounded-xl overflow-hidden bg-zinc-900/20">
          ...
          <table className="w-full text-left font-mono text-xs">
            ...
            <tbody className="divide-y divide-cyan-500/5">
              {filteredMembers.map((member) => {
                // Determine membership roles details
                  ...
                  >
                    ...
                    <td className="p-4">
                      <div className="flex flex-wrap gap-1">
                        ...
                        {memberRoles.length === 0 && (
                          <span className="text-zinc-600 text-[10px]">No clearance</span>
                        )}
                      </div>
                    </td>
                    ...
                    <td className="p-4">
                      <button
                        ...
                        className={`px-2.5 py-1 rounded text-[10px] border flex items-center gap-1.5 transition-all ${
                          member.isMuted
                    ...
                    <td className="p-4 text-right">
                      <div className="inline-flex gap-2">
                        {/* Warn Increment button */}
                        <button
                          onClick={() => handleIncrementWarning(member)}
                          disabled={member.warnings >= 3}
                          className="px-2 py-1 bg-zinc-900 border border-yellow-500/20 text-yellow-500 hover:bg-yellow-950/20 disabled:border-zinc-800 disabled:text-zinc-700 disabled:bg-transparent rounded text-[10px] flex items-center gap-1 transition-colors"
                        ...
                        {member.warnings > 0 && (
                          <button
                            ...
                            className="px-2 py-1 bg-zinc-900 border border-emerald-500/20 text-emerald-500 hover:bg-emerald-950/20 rounded text-[10px] flex items-center gap-1 transition-colors"
                          >
                            CLEAR
                          </button>
                        )}
                        ...
                        <button
                          ...
                          disabled={member.id === 'mem-4' || member.id === 'mem-1'} // Block kick F.R.I.D.A.Y or TonyStark
                          className="px-2 py-1 bg-zinc-900 border border-orange-500/20 text-orange-400 hover:bg-orange-950/20 disabled:border-zinc-800 disabled:text-zinc-700 disabled:bg-transparent rounded text-[10px] flex items-center gap-1 transition-colors"
                        ...
                        <button
                          ...
                          disabled={member.id === 'mem-4' || member.id === 'mem-1'} // Block ban F.R.I.D.A.Y or TonyStark
                          className="px-2 py-1 bg-zinc-900 border border-red-500/20 hover:border-red-500/40 text-red-500 hover:bg-red-950/30 disabled:border-zinc-800 disabled:text-zinc-700 disabled:bg-transparent rounded text-[10px] flex items-center gap-1 transition-colors"
        ...
        {filteredMembers.length === 0 && (
          <div className="p-8 text-center text-zinc-500 font-mono">
            No citizens found matching active filters in the main records directory.
          </div>
        )}
      </div>
    </div>
